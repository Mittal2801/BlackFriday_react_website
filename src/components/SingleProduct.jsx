...
const SingleProduct = () => {
    ...
    const ProductDetails = Shop_prod.find((p) => p.id === parseInt(id))
    ...
    <div className='bg-black py-20'>
      <div className='grid grid-cols-2 mx-30 text-white'>
        ...
        <div className='ms-10'>
            ...
            <div className='my-10'><span className='border p-4 px-5' onClick={() => {items.quantity > 1 ? decreaseQTY(items.id) : itemDelete(i)}}>-</span><span className='border p-4 px-5 bg-white text-gray-500'>1</span><span className='border p-4 px-5' onClick={() => increaseQTY(items.id)}>+</span><span className='bg-[#fc3746] text-white font-bold p-4 mx-10' onClick={()=>addCart(ProductDetails)}>ADD TO CART</span></div>
